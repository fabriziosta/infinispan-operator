...
func TestUpgrade(t *testing.T) {
	...
	// Approve InstallPlans and ßverify cluster state on each upgrade until the most recent CSV has been reached
...
func TestUpgradeOperatorAndRestorePreviousBackup(t *testing.T) {
	clusterName := "upgrade-operator-test"
	clusterSize := 1
	namespace := tutils.Namespace
	fmt.Printf("clusterName: %s, clusterSize: %d, namespace: %s\n", clusterName, clusterSize, namespace)
	fmt.Printf("Creating a subnamespace and the source operator\n")
	printManifest()

	labels := map[string]string{"test-name": clusterName}
	defer cleanup(labels)
	...
	infinispan.Name = clusterName
	infinispan.Labels = labels
	...
	cache.Create(config, mime.ApplicationJson)
	fmt.Printf("Populating the cluster with some data to backup\n")
	cache.Populate(numEntries)
	fmt.Printf("Assert cache size\n")
	cache.AssertSize(numEntries)
	...
	fmt.Printf("Ensure the backup pod has joined the cluster\n")
	...
	/*fmt.Printf("Delete infinispan cluster\n")
	...
	for testKube.Subscription(subscription); subscription.Status.InstalledCSV != targetChannel.CurrentCSVName; {
		...
		fmt.Printf("Approve InstallPlans\n")
		testKube.ApproveInstallPlan(subscription)
		...
		fmt.Printf("Ensure that the cluster is shutting down and restarting\n")
		testKube.WaitForInfinispanConditionWithTimeout(clusterName, tutils.Namespace, ispnv1.ConditionWellFormed, conditionTimeout)
	}

	/*fmt.Printf("Create a new infinispan cluster\n")
	testKube.Create(infinispan)
	fmt.Printf("WaitForInfinispanPods\n")
	testKube.WaitForInfinispanPods(1, tutils.SinglePodTimeout, infinispan.Name, tutils.Namespace)
	testKube.WaitForInfinispanCondition(infinispan.Name, namespace, v1.ConditionWellFormed)*/

	fmt.Printf("Restore the previous data using the backup\n")
	restoreName := clusterName + "-restore"
